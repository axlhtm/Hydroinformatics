...
##### SET THE HEADER #####logo_source = "<img src='ihelogo.png' width=auto height=100>"
logo_source = "<img src='ihelogo.png' width=auto height=100>"
logo        = Div(text=logo_source)

sgw_title   = Div(text="<b> SUGAWARA TANK MODEL </b>")
...
sub_div_title_1  = Div(text="see summary here...")
...
##### SET THE FIGURE #####
graph_sugw      = figure(plot_width=650, plot_height=250)

##### SHOW THE GUI #####

Sugawara_layout= column(column(sgw_title), row(column(d1_widget, d2_widget, s1_widget, 
...
##### figures################
graph_BWC= figure(plot_width=650, plot_height=250)
...
def on_run_btn_sugw_clicked():
    try:
            ...
            graph_sugw.line(x,q_sim)
            graph_sugw.line(x,np.array(data['Qrec']), color='red')
     ...
     try:       
            ...
            if solution_mthd.active==0:
                # explicit solution \
                  ...
                  texto.text    = 'explicit solution: L={}m, dx={}m'.format(x_total, dx)  
            ...
            graph_BWC.circle(x,wl ,size=3 )
            #graph.circle(x,wl , color='b' )
                       
            #fig.line(x,hg, color='gray' )
...
curdoc().add_root(column(logo), column(Tabs(tabs=[tab_BWC,tab_sugawara,],width=2000)))
